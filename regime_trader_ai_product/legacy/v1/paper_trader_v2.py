...
from regime_trader_ai_product.config_v2 import (
    ...
    SCAN_LIMIT, STATE_FILE
)

# 初始化日志
logger = setup_logger()
MODEL_FILE = 'regime_model_v2_quantile.pkl'  # 使用分位数模型
...
def scan_and_trade_v2():
    print(f"\n{'='*60}")
    print(f"🚀 RegimeTrader AI v2 - {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"{'='*60}\n")
    ...
    try:
        ...
        print(f"[!] Model not found: {e}. Please run: python train_model_v2.py")
        return
    ...
    print(f"[*] Equity: ${total_equity:.2f} | Cash: ${balance:.2f} | Margin: ${margin_used:.2f}")
    ...
    try:
        ...
        print(f"[!] Fetch tickers failed: {e}")
        symbols = []
