# Gate contrast — 2026-09-16 (|DI| hard floor in offline)

- Scope: offline `multi_full`, last **1y**, ETH/BNB/SOL/XRP, slip 2bps, level mode
- **Fix:** `backtest_v2_offline.py` now applies paper `|DI|≥MIN_DI_DIFF` (was DI-direction only)
- Live paper: **35 / 0.80 / 15** (conf lowered 2026-09-15)
- Runtime: 4.4s
...
1. Offline now matches paper `|DI|` floor — comparable to live gates.
2. Live **35/0.80/15** stays for sample collection; do **not** loosen DI today (24h DI-misses look like decaying trends).
3. `research_35_080_di12` is research-only; promote only after WF OOS.
4. Train overlap → relative only.
